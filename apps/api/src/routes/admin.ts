...
admin.post(
 ...
 const rawFrom =
 typeof body === "object" && body !== null && typeof (body as Record<string, unknown>).from === "string"
...
admin.post("/organizations/:orgId/subscription", adminAuthMiddleware, async (c) => {
 ...
 if (!parsed.success) throw validationError("planTier (free|starter|pro|scale) and months (1-36) required.");
 const db = getDb();
