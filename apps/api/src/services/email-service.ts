...
async function resolveInternalSender(db: DbClient, requested?: string): Promise<string> {
 ...
 .where(
 and(eq(projectTransports.projectId, INTERNAL_PROJECT_ID), eq(projectTransports.status, "active"))
 )
 .limit(20);
 const match = rows.find((r) => r.type === "gmail" && r.label.toLowerCase() === requested.toLowerCase());
 if (!match) {
 throw new AppError(
 "validation_error",
