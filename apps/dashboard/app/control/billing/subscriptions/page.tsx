  ...
  return (
    <>
      ...
      <div className="cp-stats">
        ...
        <Stat label="Past due" value={fmtInt(pastDue.length)} hint={pastDue.length ? "collection needed" : "clean"} />
        <Stat
          label="Latest"
          value={rows[0] ? fmtMoney(Number(rows[0].price ?? 0)) : "—"}
          hint={rows[0] ? `${rows[0].organization.name} · ${fmtDate(new Date(rows[0].subscription.createdAt))}` : undefined}
        />
      </div>

      <Panel title="All subscriptions" caption={`${fmtInt(rows.length)} records · latest 100`} flush>
        {rows.length === 0 ? (
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.subscription.id}>
                    ...
                    <td>
                      <Badge tone={r.subscription.status === "active" ? "ok" : r.subscription.status === "past_due" ? "warn" : undefined}>
                        <Dot tone={(TONE[r.subscription.status] as "ok" | "warn" | "bad" | "idle") ?? "idle"} />{" "}
                        {r.subscription.status}
                      </Badge>
                    </td>
                    <td className="mono" style={{ fontSize: 12.5 }}>
                      {r.subscription.currentPeriodEnd ? fmtDate(new Date(r.subscription.currentPeriodEnd)) : "—"}
                    </td>
                    <td className="mono" style={{ fontSize: 12.5, color: "var(--cp-muted)" }}>
                      {fmtDate(new Date(r.subscription.createdAt))}
                    </td>
                    <td>
                      <Link href={`/control/customers/organizations/${r.organization.id}`}>manage →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
