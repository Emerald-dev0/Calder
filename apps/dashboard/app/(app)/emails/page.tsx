...
export default async function EmailsPage({ searchParams }: { searchParams: { project?: string } }) {
  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        <div
          style={{ background: "#fff", border: "1px solid #E5E5E5", borderRadius: 12, padding: 24 }}
        >
          <p style={{ fontWeight: 600, margin: "0 0 4px" }}>Nothing sent yet</p>
          <p style={{ color: "#737373", fontSize: 14, margin: 0 }}>
            Send via the API and rows appear here with live statuses.
          </p>
        </div>
      ) : (
        <div
