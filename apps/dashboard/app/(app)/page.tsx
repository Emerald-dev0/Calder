...
export default async function OverviewPage() {
  ...
  const firstProject = ctx.memberships[0]?.projects[0] as
    { name: string; metadata?: { environment?: string } | null } | undefined;
  ...
  const cards = [
    { label: "Emails sent", value: String(stats.sent), sub: `${deliveryRate}% delivered` },
    {
      ...
      sub: `${stats.sent - stats.delivered} failed`,
    },
    { label: "In queue", value: String(stats.queued), sub: `${stats.failed} failed total` },
    {
      label: "Failed",
      value: String(stats.failed),
      sub: `${usagePct.toFixed(0)}% of ${quota.toLocaleString()}`,
    },
  ];
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--color-muted)", margin: "0 0 6px", fontSize: 13 }}>
        {firstProject
          ? `${firstProject.name} · ${(firstProject.metadata?.environment as string) ?? "Development"}`
          : ctx.memberships.length === 0
            ? "You don't belong to any organization yet."
            : `Across ${projectIds.length} project${projectIds.length === 1 ? "" : "s"}.`}
      </p>
      <p style={{ color: "var(--color-muted)", fontSize: 12, margin: "0 0 16px" }}>
      ...
      >
        ...
        >
          ...
          <p style={{ fontSize: 13, margin: "0 0 8px" }}>
            {stats.sent.toLocaleString()} / {quota.toLocaleString()}{" "}
            <span style={{ color: "var(--color-muted)" }}>{usagePct.toFixed(0)}%</span>
          </p>
          <div
            style={{
              height: 6,
          ...
          >
            <div
              style={{
                width: `${usagePct}%`,
                height: "100%",
                background: usagePct > 95 ? "#e11" : usagePct > 80 ? "#d97706" : "var(--color-ink)",
          ...
          <p style={{ fontSize: 11, color: "var(--color-muted)", margin: "8px 0 0" }}>
            {usagePct >= 95
              ? "Only " + (quota - stats.sent) + " remaining — upgrade to Pro"
              : usagePct >= 80
                ? "Approaching limit — view usage"
                : quota - stats.sent + " remaining · resets monthly"}
          </p>
        </div>
        <div
          style={{
        ...
        >
          ...
          {health.map((h) => (
            ...
            >
              <span>{h.label}</span>
              <span style={{ color: h.ok ? "#16a34a" : "#d97706", fontWeight: 600 }}>
                {h.ok ? "✓ Healthy" : "○ Setup"}
              </span>
            </div>
          ))}
