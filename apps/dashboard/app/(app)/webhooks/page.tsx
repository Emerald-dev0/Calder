    ...
    <div>
      ...
      >
        {hooks.length === 0 && (
          <p style={{ padding: 20, color: "#737373", fontSize: 14, margin: 0 }}>
            No endpoints yet. Add your URL above, deliveries, bounces, and opens will start arriving
            as signed events.
          </p>
        )}
        {hooks.map((w, i) => {
              ...
              <details style={{ marginTop: 10 }}>
                ...
                {deliveries.length === 0 ? (
                  <p style={{ fontSize: 13, color: "#737373", margin: "10px 0 4px" }}>
                    Nothing delivered yet. Send an email matching the subscribed events and it will
                    show up here with its result.
                  </p>
                ) : (
                  <table
                    style={{
      ...
      >
        ...
        <p style={{ fontSize: 13, color: "#737373", margin: "0 0 10px" }}>
          ...
          <code>t.body</code> using your endpoint's secret. Verify with timing-safe comparison and
