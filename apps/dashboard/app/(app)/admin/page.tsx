import { desc, eq, and, count, inArray, gte, sql } from "drizzle-orm";
import {
 getDb,
 users,
 emails,
...
export default async function AdminPage() {
 ...
 // Organizations with active plan + member count, for plan assignment.
 ...
 const tierById = new Map(planRows.map((p) => [p.id, p.tier]));
 const orgPlans = [];
 for (const org of orgRows.slice(0, 50)) {
 ...
 plan: active ? (tierById.get(active.planId) ?? active.planId) : null,
 periodEnd: active?.periodEnd ? new Date(active.periodEnd).toISOString().slice(0, 10) : null,
 });
 }
 ...
 <p style={{ fontWeight: 600, margin: "0 0 12px" }}>Plans, assign tiers and durations</p>
 <div
 style={{
 background: "#fff",
 border: "1px solid #E5E5E5",
 ...
 {o.members} members · {o.plan ? `${o.plan} until ${o.periodEnd}` : "no active plan"}
 </span>
 </span>
 <PlanForm orgId={o.id} currentPlan={o.plan} />
 </div>
 ))}
 </div>
