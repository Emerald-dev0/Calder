...
## ADR-029: Workspace packages ship compiled dist; API entry is import-safe

**Status:** Accepted (2026-09-15, fixes Vercel `FUNCTION_INVOCATION_FAILED` on the api project)
...
import uses `with { type: "json" }`. `apps/api/src/index.ts` default-exports
the Hono app for Vercel's Hono runtime and only calls `serve()` when executed
directly (`tsx watch`, `node dist/index.js`), never when imported.
`apps/api/vercel.json` pins `buildCommand: pnpm --filter @calder/api... build`
so workspace dist exists before functions are bundled.
**Why:** `exports: ./src/index.ts` only ever worked under `tsx`. Plain Node
(Vercel Functions, `node dist/*`, worker containers) cannot load `.ts`, so
every request crashed with `ERR_MODULE_NOT_FOUND`, and the unconditional
`serve()` call is incompatible with serverless imports. tsx, vitest, and Next
all resolve the dist + `.js`-suffix form fine, so one layout serves dev, CI,
Docker, and Vercel.
...
order `^build` first. Never point `exports` back at `src/*.ts` to "fix" dev
staleness.
